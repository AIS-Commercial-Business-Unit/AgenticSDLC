...
The dashboard now includes a **Framework Maturity Profile** radar/spider chart that shows how mature the repository is across seven framework dimensions. It complements the adoption KPI by showing not just whether assets exist, but how complete, governed, and automated they are.

### What the radar chart shows

- A 7-axis radar chart, one axis per maturity dimension
- A 0–5 score for each dimension
- A maturity label for each score band
- Expandable detail cards showing evidence, gaps, and the next recommended improvement
- An accessible table with the same underlying data

### The 7 dimensions

1. **Governance** — registry coverage, approvals, evidence rules, policy references, review cadence
2. **Agent Coverage** — how well governed agents cover the AIS lifecycle
3. **Skill Coverage** — completeness and reuse quality of skill definitions
4. **Context & Configuration** — framework configuration, prompts, templates, schemas, scoped context
5. **Quality & Verification** — tests, fixtures, schema checks, and validation workflows
6. **Delivery Automation** — CI/CD, deploy, release, metrics, and scan workflows
7. **Documentation & Enablement** — onboarding docs, playbooks, extension guides, and brownfield prompts
...
| 1 | Ad Hoc | Minimal or informal evidence |
| 2 | Emerging | Partial implementation |
| 3 | Defined | Documented and consistent |
| 4 | Enforced | Automated or actively governed |
| 5 | Measured | Measured and continuously improved |

### Old category mapping

The earlier adoption breakdown remains useful for the KPI card, but the maturity profile remaps those 8 categories into 7 richer dimensions:

| Previous category | New maturity dimension |
|---|---|
...
| `playbooks` | Documentation & Enablement |

### How to improve scores

- Add missing required artifacts first (config, prompts, templates, workflows, docs)
- Fill in metadata consistently (`approval_requirements`, `required_evidence`, `version`, `example_usage`, etc.)
- Close coverage gaps across AIS lifecycle steps
- Automate checks in GitHub Actions instead of relying on manual review
- Use the per-dimension **Gaps** and **Recommended Next Action** fields in the dashboard to prioritize the next improvement

The profile is generated by `scripts/maturity-scorer.mjs` and included in `metrics/reports/latest.json` whenever `npm run collect-metrics` runs.
